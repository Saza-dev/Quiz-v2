...
function UserQuizes() {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8070/userQuizes", quizOwner)
      .then(
        (result) => setQuizes(result.data),
        setLoading(false),
        console.log("here")
      )
  ...
  function changeStatus(QID, qIndex) {
    if (quizes[qIndex].quizStatus === "private") {
      ...
      axios
        .patch("http://localhost:8070/updateStatus", data)
        .then(
          (result) => console.log(result.data),
          setQuizStatus("public"),
              ...
              <tbody>
                {quizes.map((quiz, index) => (
                  <tr key={index}>
                    ...
                    <td
                      class="btn btn-dark pub"
                      onClick={() => changeStatus(quiz._id, index)}
                    >
                      {" "}
                      {quiz.quizStatus.toUpperCase()}{" "}
                    </td>
                    <td
                      class="btn btn-dark"
                      onClick={() => viewClicked(quiz._id)}
                    >
                      View
                    </td>
                  </tr>
